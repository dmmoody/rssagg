...
func (apiConfig *apiConfig) handlerCreateFeedFollow(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		respondWithError(w, http.StatusBadRequest, fmt.Sprintf("failed to create feed follow: %s", err))
		return
	}
...
func (apiConfig *apiConfig) handlerGetFeedFollows(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		respondWithError(w, http.StatusBadRequest, fmt.Sprintf("failed to get feed follows: %s", err))
		return
	}
...
func (apiConfig *apiConfig) handlerDeleteFeedFollow(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, fmt.Sprintf("failed to delete feed follow: %s", err))
		return
	}
